...
impl KeyError {
  ...
  fn new(key: String, kind: KeyErrorKind) -> KeyError {
    KeyError { key, kind, path: VecDeque::new() }
  }
...
impl fmt::Display for KeyError {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    ...
    match self.kind {
      KeyErrorKind::BadIndex =>
        write!(f, "bad index '{}' in list at {}", self.key, path),
      KeyErrorKind::MissingKey =>
        write!(f, "no key \"{}\" found at {}", self.key, path),
      KeyErrorKind::Dereference =>
        write!(f, "value {} is a primitive, can't access key \"{}\"", self.key, path),
      KeyErrorKind::WrongType{ref expected, ref actual} =>
        write!(f, "expected a {}, got a {} for key \"{}\" in {}", expected, actual, self.key, path),
    }
  }
}
...
enum RawConfigValue {
  /// A config value that needs no further resolution.
  CompleteValue(ConfigValue),
  // TODO(jkinkead): Interpolated strings!
  /// A reference to another key. This will not be returned from a resolved config.
  RefValue { key: String },
  /// Undefined values, only valid in templates.
  UndefinedValue(),
}
...
trait ConfigContainer<K : ToString> {
  ...
  fn get_by_path(&self, path: &[&str]) -> ConfigResult {
    ...
    Self::str_to_key(path[0]).and_then(|first_key| match self.get_at_key(&first_key) {
      ...
      None => Err(KeyError::new(first_key.to_string(), KeyErrorKind::MissingKey)),
      ...
      _ => Err(KeyError::new(first_key.to_string(), KeyErrorKind::Dereference)),
    })
  }
}
...
impl ConfigList {
  ...
  pub fn get(&self, index: usize) -> ConfigResult {
    self.values.get(index).ok_or_else(|| KeyError::new(index.to_string(), KeyErrorKind::BadIndex))
  }
...
impl ConfigMap {
  ...
  fn get_and_then<T, F: FnOnce(&ConfigValue, &[&str]) -> Result<T, KeyError>>(
      &self, key: &str, and_then: F) -> Result<T, KeyError> {
    let full_path = split_key(key);
    self.get_by_path(&full_path).and_then(|value| and_then(value, &full_path))
  }
  ...
  pub fn get_bool(&self, key: &str) -> Result<bool, KeyError> {
    ...
    self.get_by_path(&full_path).and_then(|value| match value {
      ...
      _ => Err(KeyError::wrong_type(String::from("bool"), value.kind_string(), &full_path)),
    })
  }
  pub fn get_string(&self, key: &str) -> Result<&String, KeyError> {
    let full_path = split_key(key);
    self.get_by_path(&full_path).and_then(|value| match value {
      &ConfigValue::String(ref b) => Ok(b),
      _ => Err(KeyError::wrong_type(String::from("string"), value.kind_string(), &full_path)),
    })
  }
  pub fn get_int(&self, key: &str) -> Result<i64, KeyError> {
    let full_path = split_key(key);
    self.get_by_path(&full_path).and_then(|value| match value {
      &ConfigValue::Int(b) => Ok(b),
      _ => Err(KeyError::wrong_type(String::from("int"), value.kind_string(), &full_path)),
    })
  }
  pub fn get_float(&self, key: &str) -> Result<f64, KeyError> {
    let full_path = split_key(key);
    self.get_by_path(&full_path).and_then(|value| match value {
      &ConfigValue::Float(b) => Ok(b),
      _ => Err(KeyError::wrong_type(String::from("float"), value.kind_string(), &full_path)),
    })
  }
  pub fn get_list(&self, key: &str) -> Result<&ConfigList, KeyError> {
    let full_path = split_key(key);
    self.get_by_path(&full_path).and_then(|value| match value {
      &ConfigValue::List(ref b) => Ok(b),
      _ => Err(KeyError::wrong_type(String::from("list"), value.kind_string(), &full_path)),
    })
  }
  pub fn get_map(&self, key: &str) -> Result<&ConfigMap, KeyError> {
    let full_path = split_key(key);
    self.get_by_path(&full_path).and_then(|value| match value {
      &ConfigValue::Map(ref b) => Ok(b),
      _ => Err(KeyError::wrong_type(String::from("map"), value.kind_string(), &full_path)),
    })
  }
}
...
mod tests {
  ...
  fn get_bad_key() {
    ...
    assert_eq!(result.err(), Some(
        KeyError::new(String::from("test"), KeyErrorKind::Dereference)));
  }

  #[test]
  fn get_bad_subkey() {
    let mut child_map = ConfigMap::empty();
    let mut map = ConfigMap::empty();
  ...
  #[test]
  fn get_list() {
    let mut child_list = ConfigList::empty();
    let mut map = ConfigMap::empty();
